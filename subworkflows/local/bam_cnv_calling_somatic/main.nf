...
workflow BAM_CNV_CALLING_SOMATIC {
    ...
    if (tools.split(',').contains('wakhan')) {
        wakhan_input_ch = cna_input_ch.map { tumor_bam, tumor_bai, _norm_bam, _norm_bai, snp_vcf, snp_tbi, sv_vcf ->
            tuple(tumor_bam, tumor_bai, snp_vcf, snp_tbi, sv_vcf)
        }
    ...
    if (tools.split(',').contains('savana')) {
        savana_input_ch = cna_input_ch.map { tumor_bam, tumor_bai, norm_bam, norm_bai, snp_vcf, _snp_tbi, _sv_vcf ->
            tuple(tumor_bam, tumor_bai, norm_bam, norm_bai, snp_vcf)
        }
        SAVANA_CNA(savana_input_ch, fasta, fai)
    ...
    savana_cnv = SAVANA_CNA.out.cnv // channel: [ val(meta), [ cnv_tsv ] ]
