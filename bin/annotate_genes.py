...
def check_oncokb(gsvs, oncokb):
    ...
    gsv_gene1 = pd.merge(
        gsvs, oncodat, left_on="gene_name_1", right_on="Hugo Symbol", how="left"
    )
    gsv_gene2 = pd.merge(
        gsvs, oncodat, left_on="gene_name_2", right_on="Hugo Symbol", how="left"
    )
...
if __name__ == "__main__":
    ...
    # ensure chromosomes all start with chr
    svdat = svdat[
        (svdat["chrom1"].str.startswith("chr"))
        & (svdat["chrom2"].str.startswith("chr"))
    ]
    # Assert we have data left after filtering
    assert len(svdat) > 0, "No valid SVs with 'chr' prefix found after filtering"
    ## load gene annotation
    refdat = pd.read_csv(gene_annotation, sep="\t")
    assert len(refdat) > 0, "Gene annotation file is empty"
    assert "chromosome_name" in refdat.columns, "Missing 'chromosome_name' column in gene annotation"
    assert "hgnc_symbol" in refdat.columns, "Missing 'hgnc_symbol' column in gene annotation"
    ...
    for rix, row in svs.iterrows():
        ...
        if svtype == "INS":
            ...
            for i in np.arange(1, len(brks) + 1):
                ...
                ## continue to expand the search window until we find a close match
                while gene_name == "nan" or len(gene_name) < 1:
                    gene_name, alt_gene_name = _fetch_gene_names(
                        brks[i - 1], refdat, window
                    )
                    ## stop looking and give up after expanding the search to 10kb
                    if window == 10000:
                        gene_name = "nan"
