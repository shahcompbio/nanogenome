...
def proc_rmsk(input_file, output_file):
    with open(input_file) as hin, open(output_file, "w") as hout:
        ...
        for line in hin:
            F = line.rstrip("\n").split()
            if len(F) <= 1 or F[0] in ["SW", "score", "There"]: continue
            contigs = F[4].split(",")
            contig_id, contig_len = ",".join(contigs[:-1]), contigs[-1]
            if F[8] == "C": F[8] = "-"
            ra_start, ra_end = (F[11], F[12]) if F[8] == "+" else (F[13], F[12])
            print(f"{contig_id}\t{contig_len}\t{F[5]}\t{F[6]}\t{F[8]}\t{F[9]}\t{F[10]}\t{ra_start}\t{ra_end}", file=hout)
    ...
    with open(output_file, "w") as hout:
        ...
        for read in samfile.fetch():
            ...
            if not read.is_supplementary:
                if query_strand == "+": query_start, query_end = read.query_alignment_start + 1, read.query_alignment_end
                else: query_start, query_end = query_length - read.query_alignment_end + 1, query_length - read.query_alignment_start
            else:
                if query_strand == "+": query_start, query_end = left_clip + 1, query_length - right_clip
                else: query_start, query_end = right_clip + 1, query_length - left_clip
            contigs = read.query_name.split(",")
            contig_id, contig_len = ",".join(contigs[:-1]), contigs[-1]
            print(f"{contig_id}\t{contig_len}\t{query_start}\t{query_end}\t{query_strand}\t{read.reference_name}\t{read.reference_start + 1}\t{read.reference_end}\t{read.mapping_quality}", file=hout)
        samfile.close()

if __name__ == "__main__":
    sbnd_result_txt, bwa_index_prefix, output_prefix = sys.argv[1], sys.argv[2], sys.argv[3]
    ...
    subprocess.check_call(["RepeatMasker", "-species", "human", tmp_fasta, "-dir", tmp_rmsk_dir])
    boutput_prefix = os.path.basename(output_prefix)
    proc_rmsk(os.path.join(tmp_rmsk_dir, boutput_prefix + ".tmp.fasta.out"), output_prefix + ".nanomonsv.rmsk.txt")

    with open(tmp_bwa_sam, "w") as hout:
        subprocess.check_call(["bwa", "mem", "-h", "200", bwa_index_prefix, tmp_fasta], stdout=hout)
