...
class Contig:
    ...
    def _simple_satellite(self):
        ...
        for r in self.rmsk_info:
            if r.RClass in sizes:
                sizes[r.RClass] += r.QA_End - r.QA_Start
        for cls, size in sizes.items():
            if size / self.contig_len > REPEAT_RATIO_THRES:
                self.contig_class = cls
                return

    def _plain_sv(self):
        for r in self.bwa_info:
            if (
                r.QA_Start < EARLY_THRES
                and r.QA_End - r.QA_Start >= MIN_ALIGN_LEN
                and r.MQ >= MIN_MQ
            ):
                self.contig_class = "Plain_SV"
                self.chr2 = r.TA_Chr
                self.pos2, self.dir2 = (
                    (r.TA_Start, "-") if r.QA_Strand == "+" else (r.TA_End, "+")
                )
    ...
    def _l1_mediated_del(self):
        ...
        l1_segs = [
            (r.QA_Start, r.QA_End) for r in self.rmsk_info if r.RName in L1_NAMES
        ]
        l1_inter, qsize = 0, 0
        for i in range(min(len(self.bwa_info) - 1, 2)):
            ...
            if float(l1_inter) / max(qsize, 1) <= REPEAT_RATIO_THRES:
                continue
            ...
            if nxt_len >= MIN_ALIGN_LEN and nxt.TA_Chr == self.bp_chr:
                if (
                    nxt.QA_Strand == self.bp_strand == "+"
                    and -100 < nxt.TA_Start - self.bp_pos < 10000
                ) or (
                    nxt.QA_Strand == self.bp_strand == "-"
                    and -10000 < nxt.TA_End - self.bp_pos < 100
                ):
                    self._assign_l1(nxt)
                    return
...
if __name__ == "__main__":
    sbnd_result_txt, bwa_txt, rmsk_txt, output_prefix = (
        sys.argv[1],
        sys.argv[2],
        sys.argv[3],
        sys.argv[4],
    ...
    with open(rmsk_txt) as fh:
        for F in csv.DictReader(fh, delimiter="\t"):
            ...
            if cid not in contigs:
                continue
            contigs[cid].rmsk_info.append(
                Rmsk(
                    int(F["Query_Align_Start"]),
                    int(F["Query_Align_End"]),
                    F["Query_Align_Strand"],
                    F["Repeat_NAME"],
                    F["Repeat_Class"],
                    int(F["Repeat_Align_Start"]),
                    int(F["Repeat_Align_End"]),
                )
            )
    ...
    with open(bwa_txt) as fh:
        for F in csv.DictReader(fh, delimiter="\t"):
            ...
            if cid not in contigs:
                continue
            contigs[cid].bwa_info.append(
                Bwa(
                    int(F["Query_Align_Start"]),
                    int(F["Query_Align_End"]),
                    F["Query_Align_Strand"],
                    F["Target_Align_Chromosome"],
                    int(F["Target_Align_Start"]),
                    int(F["Target_Align_End"]),
                    int(F["Mapping_Quality"]),
                )
            )
